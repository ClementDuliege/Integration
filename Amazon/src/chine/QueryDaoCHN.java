	...
	public void salarywithpaidLeave() {
		...
		try (BufferedReader reader = new BufferedReader(new FileReader(csvFile))) {
				...
				while ((line = reader.readLine()) != null) {/*On lit toutes les lignes de notre fichier*/
		           ...
		           //On stocke dans notre tableau nos différents éléments graçe à notre séparateur
		            String[]payslip = line.split(cvsSplitBy);
		            paidleave = Integer.parseInt(payslip[6]);//les congés
		            
					
		          
					name[indice]=payslip[2];//remplir les noms avec leur indice
					hg[indice] = Integer.parseInt(payslip[6]);//remplit les congés avec leur indices
		            salary[indice]=Integer.parseInt(payslip[4]);//remplit les salires avec leur indices
		            
		            indice++;
		            //On recherche les plus gros congés
		          if(payslip[6]!=" conges") {
		        ...
		        for(int i = highterPaidLeave.length-1; i>=highterPaidLeave.length-5; i--) {
		          	 ...
		          	 System.out.println("Nom : "+ firstname +"\tsalaire : " + salarys + "\tcongés :" + highterPaidLeave[i]);
		        }
	...
	public void salaryBonus() {
		...
		try (BufferedReader reader = new BufferedReader(new FileReader(csvFile))) {
				...
				while ((line = reader.readLine()) != null) {/*On lit toutes les lignes de notre fichier*/
		           ...
		           //On stocke dans notre tableau nos différents éléments graçe à notre séparateur
		            String[]payslip = line.split(cvsSplitBy);
		            bonus = Integer.parseInt(payslip[5]);//les congés
